...
app.get('/api/buscar', async (req, res) => {
  ...
  try {
    ...
    if (modo === 'rango') {
      ...
      if (r_voucher) {
        const rvRes = await poolOdoo.query(
          `SELECT number::TEXT FROM "public"."voucher_payment" WHERE trim(number) ILIKE $1`,
          [`%${r_voucher}%`]
        );
        if (rvRes.rowCount === 0) return res.json({ total: 0, pagina, por_pagina: porPag, paginas: 0, rows: [] });
        const nums = rvRes.rows.map(r => r.number);
        conditions.push(`rpd.tr_num_transferencia::TEXT = ANY($${idx++}::TEXT[])`);
        params.push(nums);
      }
    } else if (modo === 'campo' && campo && valor) {
      const like = `%${valor}%`;
      const mapCampo = {
        tramite:    `li.num_tramite_rp::TEXT LIKE $${idx}`,
        titulo:     `li.titulo_credito::TEXT LIKE $${idx}`,
        referencia: `rpd.tr_num_transferencia::TEXT LIKE $${idx}`,
        monto:      `rp.valor::TEXT LIKE $${idx}`,
        estado:     `ep.code::TEXT LIKE $${idx}`,
        tipo:       `tp.nombre::TEXT LIKE $${idx}`
      };
      if (mapCampo[campo]) {
      ...
      if (campo === 'voucher') {
        const rv = await poolOdoo.query(
          `SELECT id::TEXT as v_id FROM "public"."voucher_payment" WHERE trim("number") LIKE $1`,
          [like]
        );
        return res.json({
          total: rv.rowCount, pagina: 1, por_pagina: rv.rowCount, paginas: 1,
          rows: rv.rows.map(r => ({
            voucher_id: r.v_id, num_tramite_rp: null, titulo: null, fecha: null,
            monto: null, tr_num_transferencia: r.v_id, tipo_pago: null, estado_pago: null
          }))
        });
      }
    } else {
